...
import OrderStatusDatatable from "../profile/components/order_satus_datatable";
...
function ClientPage(props) {
  ...
  useEffect(() => {
    ...
    var_get("clients_filtered").then((data) => {
      if (data) {
        var _clients = data
          .split(",")
          .map((c) => clients.find((c_db) => c == c_db.id));
  ...
  const clients_header = () => {
    return (
      <ScrollWrapper
        className="scrollbar-none client_bar_nav gap-2 bg flex w-screen overflow-x-scroll"
        speed={400}
      >
        {clients_list.map((c, i) => {
          return (
            <div
              ...
              className={
                (c?.id == client?.id
                  ? " selected sticky top-0 z-3 border-blue-400 border-3 bg-black-alpha-10 "
                  : " z-0 border-none bg-white-alpha-10") +
                " flex cursor-pointer w-max p-2 white-space-nowrap text-white border-round-md mt-2 mb-2 hover:bg-bluegray-800 hover:text-cyan-200 transition-colors transition-duration-300"
              }
              onClick={(e) => {
                set_load_client(c);
                set_loading(true);
    ...
    return (
      ...
      >
        <div>
          ...
          {!(!show_search || loading == "client" || loading != false) && (
            <ClientSearchTable
              user={currentUser}
  ...
  return (
    ...
    >
      ...
      <div
        className={" flex w-screen h-auto align-items-center sticky left-0 bg-surface"}
      >
        <ClientSearch
          clients={clients}
      ...
      {client && (
        <div className="bg-glass-a">
          <div className="flex flex-grow-1 text-left flex-wrap w-auto h-auto gap-2 align-items-center p-2 pt-4 w-full justify-content-center">
            <h4 className="white-space-normal text-white">
              {client?.fantasia}
            </h4>
            <h5
              className=" text-overflow-ellipsis overflow-hidden hide_on_phone"
      ...
      {load_client && props.router.query.id != load_client?.id && loading && (
        <div
