...
const ClientSearch = (props) => {
  ...
  const searchClients = (query = search_query) => {
    ...
    //   _filtered_clients = [...clients];
      ...
      clients.map((item) => {
        ...
        item.score ||= 0;
        item.score += search_array
          .filter((o) => nome.includes(" " + o + " "))
          .reverse()
          .map((text, index) => similarWord(nome, text) * (index + 1))
          .concat([0])
          .reduce((a, b) => a + b);
        ...
        let any_id = search_array.filter((text) =>
          ("id_" + item.id).startsWith("id_" + text)
        );

        if (
          (("id_" + item.id).startsWith("id_" + search) &&
            search.length <= 4) ||
          any_id.length > 0
        ) {
          ret = true;
        ...
        if (
          search.includes("@") ||
          (item.email != null && item.email.toLowerCase().includes(search))
        ) {
          ret = true;
          item.score += 1;
          if (!modes.includes("Email")) modes.push("Email");
        }
        let any_phone = search_array.filter((text) =>
          item.telefone?.startsWith(text)
        );
        if (
          (search.length >= 4 &&
            item.telefone != null &&
        ...
        if (
          item.razao_social.toLowerCase().includes(search) ||
          similarText(item.razao_social.toLowerCase(), search) >= 0.5
        ) {
          ret = true;
          item.score +=
            1 + similarText(item.razao_social.toLowerCase(), search);
          if (item.razao_social.toLowerCase().startsWith(search))
            item.score += 2;
    ...
    if (_filtered_clients.length != 0) {
      let average =
        _filtered_clients.map((i) => i.score).reduce((a, b) => a + b) /
        _filtered_clients.length;
      _filtered_clients = _filtered_clients.sort((a, b) => b.score - a.score);
      let score_max = _filtered_clients[0].score;
      _filtered_clients = _filtered_clients.filter(
        (i) => i.score > average && i.score
      );
  ...
  if (props.auto_complete != false) {
    return (
      <div className="flex w-screen justify-content-center align-items-center p-2">
        <AutoComplete
          ref={autocomplete_ref}
          inputClassName="border-indigo-500 hover:border-indigo-300 text-lg font-bold uppercase"
          className="p-button-text w-screen"
          autoFocus
          // placeholder='Buscar'
          value={search_query}
          ...
          onKeyUp={(e) => {
            switch (e.key.toLowerCase()) {
              case "enter":
                ...
                autocomplete_ref.current.getInput().blur();
                break;
              default:
                break;
            }
  ...
  return (
    <div className="flex justify-content-center align-items-center p-2 w-screen">
      <span className="flex flex-grow-1 w-auto h-full p-input-icon-left p-float-label search-field">
        ...
        />
        {filter.includes("Smart") && filter.length == 1 && (
          <label
            className="hidden flex w-auto h-auto pl-2 ml-3 justify-content-center white-space-nowrap overflow-hidden text-overflow-clip"
            htmlFor="search_bar"
          >
            <span className={"mx-1 text-green-400"}>Smart</span>
          </label>
        )}
        {filter.length >= 0 && (
          <label
