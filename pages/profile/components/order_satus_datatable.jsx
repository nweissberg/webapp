...
export default withRouter(class OrderStatusDatatable extends React.Component{
	...
	header_button = 'sm:icon-only p-button-glass-dark border-none shadow-none h-3rem '
	componentDidMount(){
	...
	onViewFilter(){
		...
		if(this.state.DATE_RANGE.length>0) {
			...
			_items_filtered = _items_filtered.filter(c=>{
				return (new Date(c.EMISSAO) >= this.state.DATE_RANGE[0] && new Date(c.EMISSAO) <= this.state.DATE_RANGE[1])
			})
		}
			...
			search_array.map((term, index)=>{
				_items_filtered.map((item)=>{
				    ...
				    if(item.TELEFONE_CLIENTE.toString().indexOf(term)==0){
				        // console.log(item)
				        _search_result[item.ID_PEDIDO] = {data:item, score: search_array.length}
				    }
				})
				_items_filtered.map((item)=>{
					if(!item) return
					...
					var name_index = item.CLIENTE && normalize(item.CLIENTE).toLowerCase().indexOf(normalize(term).toLowerCase())
					if (name_index != -1) {
						_search_result[item.ID_PEDIDO] = {data:item, score: search_array.length - index}
						if(normalize(item.CLIENTE).toLowerCase().indexOf(normalize(search).toLowerCase()) != -1){
							_search_result[item.ID_PEDIDO].score += 1
						}
					}

					var vendedor_index = item.VENDEDOR && normalize(item.VENDEDOR).toLowerCase().indexOf(normalize(term).toLowerCase())
					if (vendedor_index != -1) {
						_search_result[item.ID_PEDIDO] = {data:item, score: search_array.length - index}
						if(normalize(item.VENDEDOR).toLowerCase().indexOf(normalize(search).toLowerCase()) != -1){
							_search_result[item.ID_PEDIDO].score += 1
						}
					}
				})
	...
	orderListView(){
		if(this.state.items_filtered){
			if(this.state.data_items?.length > 0){
				return(<>
					{this.state.items_filtered.sort((a,b)=>b.ID_PEDIDO-a.ID_PEDIDO).slice(0,50).map((rowData,i)=>{
						return(<div key={rowData.id+"_"+i} className="flex-wrap hover:border-blue-600 border-2 border-indigo-700 flex w-screen text-white bg-glass-a p-3 hover:bg-black-alpha-50">
							<div className="gap-3 flex flex-wrap w-screen h-full align-items-center justify-content-between">
								<div className="flex h-full w-min flex-wrap justify-content-center gap-2">
									<Button className="shadow-none p-button-outlined p-button-rounded text-purple-200 border-2 bg-black-alpha-70"
										disabled={!rowData.ID_DOCUMENTO}
										icon={rowData.view_cart=="load"?"pi pi-spin pi-spinner":(rowData.view_cart?"pi pi-eye-slash":"pi pi-eye")}
										...
										onClick={(e)=>{
											...
											if(rowData.cart.length > 0){
												...
												get_data_api({
													...
													keys:[
														{ key: "EMPRESA_ID", value: rowData.ID_FILIAL, type: "STRING"},
														{ key: "CLIENTE_ID", value: rowData.ID_CLIENTE, type: "STRING"},
														{ key: "NFE", value: rowData.ID_DOCUMENTO, type: "STRING"}
													]
												}).then((order_data)=>{
									...
									/>
									<div className="font-bold">{sqlDateToString(rowData.EMISSAO)}</div>
									<div>{time_ago(rowData.EMISSAO)}</div>
								</div>
								
								<div className="flex-grow-1 ">
									<Button className="flex shadow-none gap-2 text-gray-200 cursor-pointer align-items-center p-button-text"
									tooltip="Filtrar Cliente"
									onClick={(e)=>{
										this.setState({search:rowData.DOC_CLIENTE},this.onViewFilter)
									}}><h5 className="text-white max-w-50 h-auto">{rowData.CLIENTE}</h5>{(()=>{
									...
									<div className="flex w-full justify-content-start gap-2">
										{rowData.EMAIL_CLIENTE && rowData.EMAIL_CLIENTE!='---' && <Button className="p-button-warning p-1 flex flex-wrap p-button-text gap-3 text-left" icon="pi pi-envelope">{rowData.EMAIL_CLIENTE.split('.,').join(';').split(',').join(';').split(';').map((email)=>{
											return(<>{email.toLowerCase()}<br/></>)
										})}</Button>}
										{rowData.WHATSAPP_CLIENTE && <Button className="p-button-text p-button-success" icon="pi pi-whatsapp"
											label={rowData.WHATSAPP_CLIENTE.length == 10?format_mask(rowData.WHATSAPP_CLIENTE,"(##) ####-####"):format_mask(rowData.WHATSAPP_CLIENTE,"(##) #####-####")}
										/>}
										{rowData.TELEFONE_CLIENTE && <Button className="p-button-text p-1" icon="pi pi-phone"
											label={rowData.TELEFONE_CLIENTE.length < 10? format_mask(rowData.TELEFONE_CLIENTE,"####-####"):(rowData.TELEFONE_CLIENTE.length == 10?format_mask(rowData.TELEFONE_CLIENTE,"(##) ####-####"):format_mask(rowData.TELEFONE_CLIENTE,"(##) #####-####"))}
										/>}
										
									</div>
								</div>
								<div className="flex-wrap max-w-20rem text-gray-200">{rowData.OBSERVACAO}</div>
								<div className="justify-content-end text-right flex-grow-1">
									{rowData.ID_DOCUMENTO && <div className="font-bold text-green-300">Documento: {rowData.ID_DOCUMENTO}</div>}
									<div>{rowData.PAGAMENTO}</div>
									<div>{rowData.LANCAMENTO}</div>
									<div className="font-bold">{rowData.VENDEDOR}</div>
								</div>
								{rowData.view_cart=="load" && <div className="flex flex-wrap w-full" style={{height:"4px"}}><ProgressBar className="w-full h-full" mode="indeterminate" /></div>}
							</div>
							
							{rowData.view_cart && rowData.cart.length>0 && <div className="flex flex-wrap w-screen">
								{rowData.cart.length && rowData.cart.map((item,index)=>{
									return(<div className="grid bg-black-alpha-70 flex flex-wrap w-full max-w-screen m-1 align-items-center justify-content-between" key={index+"_"+item.produto_id}>
										<div className="flex  flex-wrap align-items-center gap-2 col-8">
											<ProductIcon size={4} item={item.produto_id}/>
											<h5>{item.nome_produto}</h5>
										</div>
										<h6 className="text-right col-4 flex-grow-1 text-right justify-content-end">Quantidade: {item.quantidade}<br/>Valor: {moneyMask(item.valor_unitario)} </h6>
									</div>)
								})}
							</div>}
						</div>)
    ...
    render(){
		...
		<div className="w-full">
			<div className="gap-3 p-3 flex w-full h-auto mb-2 justify-content-center align-items-center">
				<DateRangeMenu setDate={async(date_range)=>{
				...
				<span className="flex w-full h-full p-input-icon-left p-float-label search-field">
					...
					<InputText
						...
						onChange={(event)=>{
							...
							this.setState({search:event.target.value},this.onViewFilter)
						}}
						onKeyDown={(event)=>{
							if(event.keyCode == 13){
								// console.log("ENTER")
					...
					<label className="hidden md:flex w-auto h-auto pl-2 ml-3 justify-content-center white-space-nowrap overflow-hidden text-overflow-clip" htmlFor="search_bar">
						Buscar CLIENTE por <span className='mx-1'>Nome</span>,<span className='mx-1'>CNPJ</span> ou <span className='mx-1'>Telefone</span>
					</label>
				...
				<MultiSelect
					...
					options={this.state.data_items?.reduce((acc, item) => {
						...
						acc.result.push({ name: item.LANCAMENTO, code: item.STATUS });
						}
						return acc;
					}, { codes: new Set(), result: [] }).result}
					// itemTemplate={this.representativesItemTemplate}
				...
				{this.state.data_items?.length == 0 && <ProgressBar mode="indeterminate" />}
				{this.orderListView()}
				{/* <DataTable
					className="w-max"
					style={{width:"100%"}}
