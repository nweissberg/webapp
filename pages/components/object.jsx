...
export async function load_notifications(user_uid, show_all = false) {
  ...
  return readRealtimeData("notifications/" + user_uid).then((alerts_data) => {
    ...
    var alert_array = Object.keys(alerts_data)
      .map((alert_uid) => {
...
export default withRouter(
  class ObjectView extends React.Component {
      ...
      this.menuModel = [
        {
          ...
          command: () => this.viewAction(this.state.selectedAction),
        },
        {
          label: "Deletar",
          icon: "pi pi-fw pi-times",
          command: () => this.deleteAction(this.state.selectedAction),
        },
      ];
    ...
    componentDidMount() {
      ...
      this.onTouch = (event) => {
        ...
        let _distance = Math.sqrt(
          Math.pow(_pos.x - _cursor.x, 2) + Math.pow(_pos.y - _cursor.y, 2)
        );
    ...
    deleteAction(alert) {
      writeRealtimeData(
        "notifications/" + this.props.user.uid + "/" + alert.uid,
        null
      ).then(() => {
    ...
    onActionSelect(e) {
      this.setState({ selectedAction: e.value }, () => {
        ...
        action.viewed = true;
        writeRealtimeData(
          "notifications/" + this.props.user.uid + "/" + action.uid,
          action
        ).then((ret) => {
          this.op.hide();
    ...
    render() {
      if (this.state.redirect == true) {
        return (
          <div className="flex w-full h-full fixed justify-content-center align-items-center">
            <ProgressSpinner />
          </div>
        );
      }
      if (this.props.noUser == true)
        return (
          <>
            <div
              className="flex h-6rem w-screen justify-content-center align-items-center cursor-pointer"
              style={{
      ...
      const logotipo = () => {
        return (
          <div
            className={
              " w-full pointer-events-none h-full fixed z-2 " +
              (this.state.currentUser ? "animate-fadeout" : "animate-fadein")
            }
          >
            <img
      ...
      if (!this.state.currentUser && this.props.noUser != true)
        return logotipo();
      return (
          ...
          {users_in_room.length > 0 && this.props?.show_users == true && (
            <div style={{ position: "absolute", zIndex: 1 }}>
              {users_in_room.map((user, uindex) => {
                ...
                var is_on =
                  (!user_position || Date.now() - user_position.time > 60000) ==
                  false;
                if (p_x < window.innerWidth && p_y < window.innerHeight) {
                  return (
                    ...
                    >
                      ...
                      <Badge
                        value={is_on ? "ON" : "OFF"}
                        style={{ scale: "0.9", transform: "translateY(25px)" }}
                        className={
                          "z-0 hover:visible cursor-pointer absolute shadow-8 bottom-0 text-white " +
                          (is_on ? "bg-green-700" : "bg-gray-600")
                        }
                      />{" "}
                    </div>
                  );
              ...
              <Toolbar
                ...
                left={
                  <div className="flex align-items-center m-0 p-0">
                    {this.props.alerts != false && this.props.user && (
                      <div style={{ position: "absolute" }}>
                        <i
                          ...
                          onClick={(event) => {
                            ...
                            load_notifications(this.props.user.uid).then(
                              (alerts_data) => {
                        ...
                        >
                          {this.state.alerts?.length > 0 && (
                            <Badge value={this.state.alerts?.length} />
                          )}
                        </i>
                      </div>
                    )}
                        ...
                        >
                          <Column
                            ...
                            body={(row_data) => {
                              return (
                                <div>
                                  <h5 style={{ color: "var(--text-c)" }}>
                                    {row_data.title}
                                  </h5>
                                  <div>
                                    <div className="pl-1">
                                      {row_data.short
                                        ? shorten(row_data.message, 6, false)
                                        : row_data.message}
                                    </div>
                                    {row_data.message.length > 64 && (
                                      <Button
                                        label={
                                          row_data.short ? "Ler mais" : "Menos"
                                        }
