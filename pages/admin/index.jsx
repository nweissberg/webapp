...
export default function AdminPage() {
  ...
  useEffect(() => {
    ...
    readUsers().then(async (data) => {
      ...
      for (const [key, value] of Object.entries(data)) {
        ...
        await get_user_orders(key).then((orders_data) => {
      ...
      data = Object.values(data).filter((user) => {
        if (user.photo.indexOf("super") == -1) {
          if (
            ...
            user.role == 0 ||
            currentUser.email === process.env.NEXT_PUBLIC_MASTER
          ) {
            return user;
          }
        }
  ...
  return (
    ...
    >
      ...
      <main className="flex flex-wrap justify-content-center w-screen">
        <div className="absolute bg-glass-b w-full ">
          <div className="w-auto overflow-auto">
                      ...
                      return (
                        <div className="flex justify-content-center">
                          <Button
                            ...
                            label={
                              row_data.orders.length +
                              " Orçamento" +
                              (row_data.orders.length > 1 ? "s" : "")
                            }
                            onClick={async (event) => {
                    ...
                    return (
                      <div className="flex gap-2 align-items-center">
                        <Button
                          ...
                          tooltip={
                            row_data.parent
                              ? row_data.parent.map((u) => u.name).join("\n")
                              : ""
                          }
                          icon="pi pi-sitemap"
                          onClick={(event) => {
                        ...
                        {row_data.parent && (
                          <h6>
                            {row_data.parent.length == 1
                              ? row_data.parent[0].name
                              : row_data.parent.length + " usuários"}
                          </h6>
                        )}
                      </div>
                    );
                      ...
                      body: (
                        ...
                        >
                          ...
                          <Column
                            ...
                            body={(row_data) => {
                              return (
                                <div className="flex flex-wrap w-11rem">
                                  <div className="flex align-items-center w-auto h-full">
                                    <label className="text-purple-300 font-bold uppercase">
                                      {row_data.name}
                                    </label>
                                    <Button
                                      className="text-purple-100 p-button-text shadow-none p-button-rounded p-button-lg"
                            ...
                            body={(row_data) => {
                              return (
                                <MultiSelect
                                  ...
                                  selectAll
                                  value={
                                    row_data.dashboard ? row_data.dashboard : []
                                  }
                          ...
                          <Column
                            ...
                            body={(row_data) => {
                              ...
                              return (
                                <div className="flex-grow-1 h-auto">
                                  <InputNumber
                                    suffix=" %"
                                    // style={{width:"100%"}}
                                    value={row_data.discount}
                      ...
                      body: (
                        <div>
                          ...
                          >
                            <Column
                              header="Imagem"
                              field="dashboard_image"
                              body={(row_data) => {
                                return (
                                  <Button
                                    className="hover:opacity-100 p-0 max-w-10rem max-h-10rem border-round-lg border-none overflow-hidden justify-content-center"
                                    onClick={(e) => {
                                      // console.log(row_data)
                            ...
                            <Column
                              ...
                              body={(row_data) => {
                                return (
                                  <UserIcon uid={row_data.user_uid} inline />
                                );
                              }}
                            />
                            <Column
                              ...
                              body={(row_data) => {
                                return (
                                  <div>
                                    <div>
                                      {sqlDateToString(
                                        row_data.enviado?.toDate()
                                      )}
                                    </div>
                                    <div>
                                      {time_ago(row_data.enviado.toDate())}
                                    </div>
                                  </div>
                                );
                              }}
            ...
            <Dialog
              header={
                selected_user
                  ? `Fluxo de aprovação de ${selected_user.name}`
                  : "Fluxo"
              }
