...
import { deepEqual, fetchWithTimeout, format_mask, isDeepEqual, moneyMask, print, scrollToBottom, scrollToTop, var_get, var_set } from "../../utils/util";
...
export default class SalesFooter extends React.Component{
                    ...
                    },
                    {
                        ...
                        command:(e) => {
                            ...
                            var _order = {
                                ...
                                client:{fantasia:this.props.client.fantasia, id:this.props.client.id},
    ...
    send(e) {
        ...
        const items_string = JSON.stringify(_order.items.map((produto,index)=>{
            return({
        ...
        var_get('api_token',{
            ...
            if(update){
                ...
                await api_cloud.get('token').then(api_token=>{
                    ...
                    var_set("api_token", JSON.stringify(api_token),{ session:true, compress:true })
                })
            }
    ...
    componentDidUpdate(){
        ...
        if(this.state.save_name == "" && this.state.save_name != this.props.sale_cart.name && this.props.sale_cart.name != ""){
            this.setState({save_name:this.props.sale_cart.name, isSaved:true})
        }else if(this.state.isSaved){
            pedidos_db.getItem(this.props.user.uid).then((data)=>{
                if(data){
                    const current_cart = data.drafts.find(cart=>cart.name == this.props.sale_cart.name)
                    const isEqual = deepEqual(this.props.sale_cart.items,current_cart?.items)
                    if(isEqual == false){
